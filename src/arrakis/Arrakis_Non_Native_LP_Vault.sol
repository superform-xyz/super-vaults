...
contract ArrakisNonNativeVault is ERC4626 {
    ...
    constructor(
        ...
        ERC4626(
            ERC20(isToken0_ ? address(IGUniPool(gUniPool_).token0()) : address(IGUniPool(gUniPool_).token1())),
            name_,
            symbol_
        )
    {
    ...
    function beforeWithdraw(uint256 underlyingLiquidity_, uint256) internal override {
        /// @notice getting the pool liquidity from arrakis vault
        IUniswapV3Pool uniPool = arrakisVault.pool();
        (uint128 liquidity_,,,,) = uniPool.positions(arrakisVault.getPositionID());
        uint256 sharesToWithdraw = (underlyingLiquidity_ * arrakisVault.totalSupply()) / liquidity_;
        ...
        swapParams memory params = swapParams({
            ...
            sqrtPrice: !zeroForOne ? sqrtPriceX96 - (twoPercentSqrtPrice) : sqrtPriceX96 + (twoPercentSqrtPrice),
            data: ""
        });
    ...
    function _viewUnderlyingBalanceOf() internal view returns (uint256) {
        ...
        if (zeroForOne) {
            ...
            grossLiquidity = (
                (
                    ((priceDecimals) * ((gross1 * (10 ** asset.decimals())) / (10 ** non_asset.decimals())))
                        / (10 ** asset.decimals())
                ) + gross0
            );
            liquidity = (grossLiquidity * gauge.balanceOf(address(this))) / arrakisVault.totalSupply();
        } else {
            priceDecimals = ((10 ** non_asset.decimals()) * ((sqrtRatioX96 * sqrtRatioX96) / X96)) / X96;
            grossLiquidity = (
                ((priceDecimals) * ((gross0 * (10 ** asset.decimals())) / (10 ** non_asset.decimals())))
                    / (10 ** asset.decimals()) + gross1
            );
            liquidity = (grossLiquidity * gauge.balanceOf(address(this))) / arrakisVault.totalSupply();
        }
        return liquidity;
    ...
    function afterDeposit(uint256 underlyingAmount_, uint256) internal override {
        ...
        arrakisRouter.addLiquidityAndStake(gauge, token0Bal, token1Bal, amount0Used, amount1Used, address(this));
    }
    ...
    function withdraw(uint256 assets_, address receiver_, address owner_) public override returns (uint256 shares) {
        uint256 liquidity = previewWithdraw(assets_);
        /// @dev No need to check for rounding error, previewWithdraw rounds up.
    ...
    function totalLiquidity() public view returns (uint256) {
        (uint128 liquidity_,,,,) = arrakisVault.pool().positions(arrakisVault.getPositionID());
        uint256 liquidity = (gauge.balanceOf(address(this)) * liquidity_) / arrakisVault.totalSupply();
        return liquidity;
    }
    ...
    function previewRedeem(uint256 shares_) public view override returns (uint256) {
        ...
        /// @dev Saves an extra SLOAD if totalSupply is non-zero.

        return supply == 0 ? shares_ : shares_.mulDivDown(totalLiquidity(), supply);
    }
    ...
    function previewWithdraw(uint256 assets_) public view override returns (uint256) {
        ...
        return supply == 0 ? assets_ : assets_.mulDivUp(totalLiquidity(), totalAssets());
    }
    ...
    function uniswapV3SwapCallback(int256 amount0Delta_, int256 amount1Delta_, bytes calldata /*data*/ ) external {
        if (msg.sender != address(arrakisVault.pool())) {
            revert NOT_UNIV3_POOL_CALLBACK();
        }

        if (amount0Delta_ > 0) {
            ERC20(address(arrakisVault.token0())).safeTransfer(msg.sender, uint256(amount0Delta_));
        } else if (amount1Delta_ > 0) {
            ERC20(address(arrakisVault.token1())).safeTransfer(msg.sender, uint256(amount1Delta_));
        }
    }
        ...
        returns (bool direction, uint256 swapAmount)
    {
        uint256 amount0Left;
        uint256 amount1Left;
        try pool_.getMintAmounts(amount0In_, amount1In_) returns (uint256 amount0, uint256 amount1, uint256) {
        ...
        uint256 weightX18 = FullMath.mulDiv(gross0 * factor0, 1 ether, gross1 * factor1);
        uint256 proportionX18 = FullMath.mulDiv(weightX18, price18Decimals_, 1 ether);
        uint256 factorX18 = FullMath.mulDiv(proportionX18, 1 ether, proportionX18 + 1 ether);

        if (amount0Left > amount1Left) {
            direction = true;
            swapAmount = FullMath.mulDiv(amount0Left, 1 ether - factorX18, 1 ether);
        } else if (amount1Left > amount0Left) {
            swapAmount = FullMath.mulDiv(amount1Left, factorX18, 1 ether);
        }
    }

    function _getUnderlyingOrLiquidity(IGUniPool pool_) internal view returns (uint256 gross0, uint256 gross1) {
        ...
        if (gross0 == 0 && gross1 == 0) {
            ...
            (gross0, gross1) =
                LiquidityAmounts.getAmountsForLiquidity(sqrtPriceX96, lowerSqrtPrice, upperSqrtPrice, 1 ether);
        }
    }

    function _swap(swapParams memory params_) internal {
        IUniswapV3Pool uniPool = arrakisVault.pool();
        uniPool.swap(params_.receiver, params_.direction, params_.amount, params_.sqrtPrice, params_.data);
    }
}

...
import {
    ...
    TickMath,
    SafeTransferLib
} from "../Arrakis_Non_Native_LP_Vault.sol";
...
contract Arrakis_LP_Test is Test {
    ...
    function paramswap() external {
        ...
        uniPool.swap(
            ...
            blah,
            int256(amount),
            blah ? sqrtPriceX96 - (twoPercentSqrtPrice) : sqrtPriceX96 + (twoPercentSqrtPrice),
            ""
        );
        blah = !blah;
    }
        ...
        returns (uint256 fee)
    {
        ...
        if (isZero) {
            ...
            (,, feeGrowthOutsideLower,,,,,) = pool.ticks(gUniPool.lowerTick());
    ...
    function getUnderlyingBalances() external view returns (uint256 amount0Current, uint256 amount1Current) {
        ...
        (uint128 liquidity,,,,) = gUniPool.pool().positions(gUniPool.getPositionID());
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata /*data*/ ) external {
        ...
        if (amount0Delta > 0) {
            ERC20(address(gUniPool.token0())).safeTransfer(msg.sender, uint256(amount0Delta));
        } else if (amount1Delta > 0) {
            ERC20(address(gUniPool.token1())).safeTransfer(msg.sender, uint256(amount1Delta));
        }
    }

    function swap(uint256 amtIn, address[] memory path) internal returns (uint256) {
        ERC20(path[0]).approve(address(joeRouter), amtIn);
        uint256[] memory amts = joeRouter.swapExactTokensForTokens(amtIn, 0, path, address(this), block.timestamp + 1);
    ...
    function testDepositWithToken0AsAssetSuccess() public {
        ...
        uint256 sharesReceived = arrakisNonNativeVault.deposit(amt, address(this));
        console.log("Underlying balance :", vault.totalAssets());
        /// @dev we simulate the swaps on the same pool we are adding liquidity to, so we can get the fees accrued and
        /// test the reinvest.
        console.log("Starting swap simulation on uniswap....");
        uint256 countLoop = 2;
        ERC20(address(WMATIC)).transfer(address(this), 298_000e18);
        while (countLoop > 0) {
            this.paramswap();
            countLoop--;
        }
        ...
        this.computeFeesAccrued();
        uint256 returnAssets = arrakisNonNativeVault.redeem(sharesReceived, address(this), address(this));
        console.log("Underlying balance :", vault.totalAssets());
        emit log_named_decimal_uint(
            "amount gained through out the duration in the form of deposited Asset", returnAssets, 18
        );
    }
    ...
    function testMintWithToken0AsAssetSuccess() public {
        ...
        this.computeFeesAccrued();
        uint256 returnAssets = arrakisNonNativeVault.withdraw(vault.totalAssets() - 1e19, address(this), address(this));
        console.log("Underlying balance :", vault.totalAssets());
        emit log_named_decimal_uint(
            "amount gained through out the duration in the form of deposited Asset", returnAssets, 18
        );
    ...
    function testDepositWithToken1AsAssetSuccess() public {
        ...
        uint256 sharesReceived = arrakisToken1AsAssetVault.deposit(2000 * (10 ** 6), address(this));
        console.log("Underlying balance :", vault.totalAssets());
        console.log("Shares received:", sharesReceived);
        console.log("Starting swap simulation on uniswap....");
        uint256 countLoop = 2;
        ERC20(address(USDC)).transfer(address(this), ERC20(address(USDC)).balanceOf(address(this)));
        while (countLoop > 0) {
            this.paramswap();
            countLoop--;
        }
        ...
        uint256 returnAssets = arrakisToken1AsAssetVault.redeem(sharesReceived, address(this), address(this));
        console.log("Underlying balance :", vault.totalAssets());
        emit log_named_decimal_uint(
            "amount gained through out the duration in the form of deposited Asset", returnAssets, 6
        );
    }
